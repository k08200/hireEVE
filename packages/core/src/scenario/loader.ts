...
export function loadScenario(filePath: string): Scenario {
  ...
  if (!data || typeof data !== "object") {
    throw new Error(
      `Invalid scenario format in ${filePath}: expected a YAML object, got ${typeof data}`,
    );
  }
...
function validateScenario(data: Record<string, unknown>): string[] {
  ...
  if (!data.steps || !Array.isArray(data.steps)) {
    ...
    for (let i = 0; i < data.steps.length; i++) {
      ...
      if (!step.action || typeof step.action !== "string") {
        ...
        errors.push(
          `steps[${i}]: unknown action '${step.action}' (valid: ${validActions.join(", ")})`,
        );
      }
    }
  }
  ...
  if (!data.evaluate || typeof data.evaluate !== "object") {
    ...
    if (!evaluate.method || typeof evaluate.method !== "string") {
      ...
      errors.push(
        `evaluate.method must be one of: ${validMethods.join(", ")} — got '${evaluate.method}'`,
      );
    }
...
export function loadScenarios(filePaths: string[]): Scenario[] {
  ...
  if (errors.length > 0 && scenarios.length === 0) {
    throw new Error(
      `All ${filePaths.length} scenario file(s) failed to load:\n${errors.join("\n")}`,
    );
  }
