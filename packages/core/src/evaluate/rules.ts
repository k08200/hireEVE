    ...
    case "exit_code": {
      ...
      if (Number.isNaN(expected)) {
        return {
          rule,
          passed: false,
          detail: `Invalid exit_code value "${rule.value}" — must be an integer`,
        };
      }
...
function resolveTarget(target: string, output: RunOutput): { value: string; error?: string } {
  ...
  return {
    value: "",
    error: `Unknown target "${target}" — valid targets: stdout, stderr, exit, file:<path>`,
  };
}
